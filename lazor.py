...
def find_fixed_positions(small_grid):
    return [[i * 2 + 1,
             j * 2 + 1] for i,
            row in enumerate(small_grid) for j,
            block in enumerate(row) if block in {'A',
                                                 'B',
                                                 'C'}]
...
def solve_path(
    ...
    blocks = ['A'] * num_reflective + ['B'] * \
        num_opaque + ['C'] * num_refractive
    blocks += ['o'] * (sum(row.count('o') for row in grid) - len(blocks))

    # Iterate over each permutation of the block arrangement
    for blocks_temp in multiset_permutations(blocks):
        test_board = Grid_Structure(grid).generate_grid(
            blocks_temp, fixed_positions)
